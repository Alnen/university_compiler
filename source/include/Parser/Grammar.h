...
public:
    ...
    using RuleList = std::vector<Rule<TerminalType, NonterminalType>>;
    ...
    static bool isTerminal(TokenType symbol);
    static bool isNonterminal(TokenType symbol);
    static bool isAction(TokenType symbol);
...
bool Grammar<Terminal, Nonterminal>::isTerminal(TokenType symbol)
{
    return Terminal::TERMINAL_START_INDEX < symbol && symbol < Terminal::TERMINAL_FINISH_INDEX;
}

template <class Terminal, class Nonterminal>
bool Grammar<Terminal, Nonterminal>::isNonterminal(TokenType symbol)
{
    return Nonterminal::NONTERMINAL_START_INDEX < symbol && symbol < Nonterminal::NONTERMINAL_FINISH_INDEX;
}

template <class Terminal, class Nonterminal>
bool Grammar<Terminal, Nonterminal>::isAction(TokenType symbol)
{
    return Nonterminal::ACTION_START_INDEX < symbol && symbol < Nonterminal::ACTION_FINISH_INDEX;
}
...
Grammar<Terminal, Nonterminal>::ruleFirst(std::vector<TokenType> A) const
{
    ...
    {
        if (!checked_rules.contain(A[i]) && !isAction(A[i]))
        {
            TokenSet f_set = first(A[i], checked_rules);
            if (f_set.size() == 0)
            {
                throw std::runtime_error("No rules for this nonterminal!");
            }

            if (!f_set.contain(TokenType(Nonterminal::EPSILON)) || (i == (A.size() - 2)))
            {
    ...
    else
    {
        ...
        for (size_t i = 0; i < rules.size(); ++i)
        {
            if (rules[i].left() == A)
            {
                ...
                {
                    if (!checked_rules.contain(rules[i].right()[j]) && !isAction(rules[i].right()[j]))
                    {
                        ...
                        if (!f_set.contain(TokenType(Nonterminal::EPSILON)) || (j == (rules[i].right().size() - 2)))
                        {
                            first_set.update(f_set);
                            break;
                        }
    ...
    for (size_t i = 0; i < rules.size(); ++i)
    {
        for (size_t j = 0; j < rules[i].right().size(); ++j)
        {
            if (rules[i].right()[j] == A)
            {
                ...
                {
                    TokenList rest_of_rule(rules[i].right().begin() + (j + 1), rules[i].right().end());
